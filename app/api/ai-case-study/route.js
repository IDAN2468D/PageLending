...
export async function POST(req) {
    ...
    try {
        ...
        const prompt = `
אתה קופירייטר שיווקי של "PageLending" שמספק ייעוץ פיננסי.
הלקוח שפנה כרגע מזדהה בתור: "${personaName}" (${personaDesc}).

צור מיד "מקרה בוחן" (Case Study) פיקטיבי אבל ריאליסטי מאוד, המתאר לקוח עבר של PageLending שהיה בדיוק באותו מצב. 
...
החזר את התשובה *אך ורק* כ-JSON במבנה הבא (בלי מארקדאון):
{
  ...
  "before_situation": "משפט אחד לתאר את המצב המלחיץ/תקוע לפני הפנייה ל-PageLending",
